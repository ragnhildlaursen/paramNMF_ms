...
init = 500   # number of initialisations
smallIter = 500
tol = 1e-8
...
for(nsim in 1:nSim){
  ...
  sampleV = matrix(rpois(nG*nMtTps, estimate), nrow = nG)
  ResultFixMix <- nmfprm(data = sampleV, noSignatures = noSig, designMatrices = list(Mmono,Mfull,Mdi,Mdi),tol=tol,initial = init, smallIter = smallIter)
  ...
  save(ResCosineMatTri,ResCosineMatMix, ResCosineMatDi,ResCosineMatMono, file = "SimilaritySignaturesBRCA21.RData")
}
